...
cdef class BinnedLH_hist:
    ...
    cdef readonly bint use_w2
    cdef int nint_subdiv
    def __init__(self, f, data, bins, weights=None, weighterrors=None,
            badvalue=1000000, extended=False, use_w2=False, nint_subdiv=1):
        """
        Create a Poisson Binned Likelihood object from given PDF **f** and
        **data** (histograms). Constant term and expected minimum
        are subtracted off (aka. log likelihood ratio). The exact calculation
        will depend on **extended** and **use_w2** keyword parameters.
        
        - version for use with histogramed data
        ...
        **Arguments**
            ...
            - **data** 1D array of data. This is an histogrammed
              data.
        ...
        self.w2 = float2double(self.h)
        self.midpoints = mid(self.edges)
        self.binwidth = np.diff(self.edges)

        #self.bins = bins
        self.badvalue = badvalue
        self.ndof = len(self.h)-(self.func_code.co_argcount-1)
